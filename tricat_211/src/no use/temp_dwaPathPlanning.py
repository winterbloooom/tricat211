...
class Boat:
    def __init__(self):
        ...
        self.yaw = 0.0  # ex ) math.pi/8.0  # [rad] current rotation(yaw) of boat
...
class DWA_Calc:
    ...
    # def set_goal_points(self, next_goal):
    #     self.goal = next_goal

    def set_obstacle_list(self, detected_obstacles):
        self.obstacles = np.array(detected_obstacles)
    ...
    def calc_obstacle_cost(self, trajectory):
        obstacle_x = self.obstacles[:, 0]
        obstacle_y = self.obstacles[:, 1]
        dx = trajectory[:, 0] - obstacle_x[:, None]
        dy = trajectory[:, 1] - obstacle_y[:, None]
        r = np.hypot(dx, dy)
...
class Ob:
    def __init__(self):
        ...
        rospy.Subscriber('/filtering_obstacles', FilteringObstacles, self.filtering_obstacles_callback)

    def filtering_obstacles_callback(self, msg):
        self.f_obstacle_list = msg.circles
        for i in range(len(f_obstacle_list)):
            self.f_obstacle_list_x.append(self.f_obstacle_list[i].x)
            self.f_obstacle_list_y.append(self.f_obstacle_list[i].y)
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        dwa_path.set_obstacle_list(obstacle.f_obstacle_list)
